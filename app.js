...
const intializeDbAndServer = async () => {
  try {
    ...
    console.log(`Db Errot ${error.message}`)
    process.exit(1)
  }
}
...
const directorTableconvertDbObjectToResponseObject = dbObject => {
  return {
    directorId: dbObject.director_id,
...
app.get('/directors/', async (request, response) => {
  ...
  response.send(
    directorArray.map(eachdirector =>
      directorTableconvertDbObjectToResponseObject(eachdirector),
    ),
  )
})
